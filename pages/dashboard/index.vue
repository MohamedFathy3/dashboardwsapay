<template>
  <section v-if="authStore.isMember" class="member-dashboard">
        ...
        <div class="card-body">
          <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
            <div>
              ...
              <h2 class="member-title mb-1">Welcome, {{ authStore.displayName }}</h2>
              ...
              <div v-else class="member-transactions">
                ...
                >
                  ...
                  <div class="text-right">
                    <strong
                      :class="[
                        transaction.type === 'add' ? 'text-success' : 'text-danger'
                      ]"
                    >
...
const memberBalances = computed<MemberBalance[]>(() => {
  const balances = ((authStore.user as any)?.balances || (authStore.user as any)?.wallets || []) as Array<{
    currency: string;
...
const loadApprovedRecipients = async () => {
  try {
    const response = await useApiFetch<{ data?: MemberRecipient[]; meta?: any }>(`/members/index`, {
      method: "POST",
      body: {
        paginate: false,
        filters: {
    ...
    approvedRecipients.value = (response?.data || []).filter(
      (member) => String(member.id) !== String(authStore.user?.id)
    );
  } catch (error) {
...
const confirmTransfer = async () => {
  ...
  try {
    ...
    transferError.value = /balance.*0\.01/i.test(backendMessage)
      ? "The backend is rejecting automatic recipient wallet creation because that endpoint requires an initial balance greater than 0.01. Your transfer amount is not the problem."
      : /wallet not found/i.test(backendMessage)
        ? "The recipient does not have a wallet for this currency yet. This must be handled by the backend transfer flow so the wallet can be created automatically on receive."
        : backendMessage;
...
const refreshMemberDashboard = async () => {
  await Promise.all([loadMemberProfile(), loadApprovedRecipients()]);
};
